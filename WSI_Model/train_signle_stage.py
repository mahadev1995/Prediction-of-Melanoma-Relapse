...
# Check if checkpoint and history directories exist, and create them if not
...
trainer = Trainer(net, criterion1, criterion2, criterion3, scheduler, optimizer, train_loader, val_loader, True, False)

# Start training the model
loss_history, avg_val_loss_relapse = trainer.train(start_epoch=start_epoch, end_epoch=end_epoch, checkpoint_interval=save_interval, checkpoint_path=ckpt_path, history_path=history_path, lamda=lmbda)

# Create a DataFrame to store the loss history and save it to a CSV file
loss_df = pd.DataFrame(loss_history, columns=['TrainLoss', 'LossRelapse', 'LossMeta', 'ValLoss', 'ValLossRelapse', 'ValLossMeta', 'ValAcc', 'ValF1', 'ValAucRuc'])
